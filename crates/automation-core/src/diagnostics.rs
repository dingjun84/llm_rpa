...
pub struct Observation<'a> {
    ...
    /// 这一帧所在的**整窗**（窗口截图 + 它在屏幕上的矩形），拿不到时为 `None`。
    ///
    /// ## 为什么要有它
    ///
    /// 只给 `frame`（裁图）时，标注图上那圈「要识别的区域」恒等于图像边界——
    /// **看不出这块区域落在窗口的哪个位置**，而"区域标定偏了"恰恰是最常见的一类故障。
    /// 把整窗一起交出来，才画得出「窗口 + 区域框」这张能一眼判定的图。
    ///
    /// 取不到（还没进客户端、演练替身、截屏失败）时退回只画裁图：诊断**永远不该**
    /// 让任务失败，所以这是尽力而为的字段。
    pub window: Option<WindowShot<'a>>,
    /// 识别到的文字块；只截了指纹、没做 OCR 的步骤为空。
    pub text_boxes: &'a [TextBox],
    /// 图标匹配的命中结果；不做图标匹配的步骤为 `None`。
    pub icon: Option<IconHit>,
...
/// 一步所在窗口的**整窗截图**：窗口矩形 + 那一帧画面。
///
/// ## 为什么诊断图要拿它当底图
///
/// 只给裁图（[`Observation::frame`]）时，图上那圈「要识别的区域」恒等于图像边界，
/// 画出来等于什么都没说——**看不出这块区域落在窗口的哪个位置**。
/// 而"区域标定偏了"（框到了头像列、框到了标题栏）恰恰是最常见的一类故障。
/// 有整窗当底图，区域框才落在一个能一眼判定的位置上。
///
/// ## 为什么是尽力而为
///
/// 还没进客户端、演练替身、截屏失败时都取不到。诊断**永远不该**让任务失败，
/// 所以这是个可选字段：拿不到就退回只画裁图（那时区域框仍等于图像边界）。
pub struct WindowShot<'a> {
    /// 窗口在**屏幕**上的矩形（逻辑坐标），与 [`Rect`] 的约定一致。
    pub rect: Rect,
    /// 整窗画面（BGRA），与 [`Screenshot`] 的约定一致。
    pub frame: &'a Screenshot,
}
...
/// ## 为什么图标命中要单独画出来
///
/// 图标上**没有文字**，OCR 那一套（文字框 + 右侧文字栏）对它完全不适用。
/// 而"它把哪个图标认成了目标"是这一步唯一的可读结果——不画出来，
/// 复盘时手上只剩一个坐标，无从判断那地方到底是不是那个图标。
#[derive(Debug, Clone)]
pub struct IconHit {
    /// 命中框，位于**本帧图像坐标**（同 [`TextBox::bounds`]，原点即区域左上角）。
    pub bounds: Rect,
