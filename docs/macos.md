...
4. 「记录窗口尺寸」→「截取目标窗口」完成区域标定（走搜索式还要标
   `main_search` / `search_dropdown` / `contact_profile` / `send_button` 四块）；
5. 编译 `macosocr` 并填入 `ocr_command`；
6. 先开「只填不发」跑通：定位联系人 → 输入框有字 → 终态 `Prepared`；
7. 再关「只填不发」发一条测试消息——**不需要任何人确认**：程序填完正文就直接点
   「发送按钮区」里认出的那个按钮发出去。发送动作是**逐字输入正文 + 点按钮**，
   所以第 4 步那块区域标得准不准，这一步才知道。
